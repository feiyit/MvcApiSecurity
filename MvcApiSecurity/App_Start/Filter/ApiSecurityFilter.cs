...
namespace Filter
{
    public class ApiSecurityFilter : ActionFilterAttribute
    {
        public override void OnActionExecuting(System.Web.Http.Controllers.HttpActionContext actionContext)
        {
            ...
            string appkey = String.Empty, timestamp = string.Empty, nonce = string.Empty, signature = string.Empty, signKey=string.Empty;
            //客户的唯一标示
            ...
            {
                signKey = HttpUtility.UrlDecode(request.Headers.GetValues("signKey").FirstOrDefault());
            }
            //签名
            ...
            if (actionContext.ActionDescriptor.ActionName == "GetToken")
            {
                if (string.IsNullOrEmpty(appkey) || (string.IsNullOrEmpty(timestamp) || string.IsNullOrEmpty(nonce)))
                {
                    ...
                    resultMsg.Data = "";
                    actionContext.Response = HttpResponseExtension.toJson(Common.JsonConvert.Serialize(resultMsg));
            ...
            //判断请求头是否包含以下参数
            if (string.IsNullOrEmpty(appkey) || (string.IsNullOrEmpty(timestamp) || string.IsNullOrEmpty(nonce) || string.IsNullOrEmpty(signature) || string.IsNullOrEmpty(signKey)))
            {
                resultMsg = new ResultMsg();
                resultMsg.StatusCode = (int)StatusCodeEnum.ParameterError;
                resultMsg.Info = StatusCodeEnum.ParameterError.GetEnumText();
            ...
            {
                case "POST":
                    NameValueCollection pv = HttpContext.Current.Request.Form;
                    //对参数和签名Key组合data加密
                    string signStr = string.Empty;
                    for (int f = 0; f < pv.Count; f++)
                    {
                        string key = pv.Keys[f];
                        signStr += key + signKey + pv[key];
                    }
                    data = signStr;
                    break;
                case "GET":
            ...
            bool result = SignExtension.Validate(timestamp, nonce, signtoken,data, signature);
            if (!result)
            {
                resultMsg = new ResultMsg();
